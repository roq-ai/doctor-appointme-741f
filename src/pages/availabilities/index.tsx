...
type ColumnType = ColumnDef<AvailabilityInterface, unknown>;
...
export function AvailabilityListPage(props: AvailabilityListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getAvailabilities({
        ...
        order: params.order,
        searchTermKeys: ['day_of_week.contains', 'status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AvailabilityInterface>>(
    () => `/availabilities?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AvailabilityInterface) => {
    if (hasAccess('availability', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/availabilities/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'day_of_week', header: 'Day Of Week', accessorKey: 'day_of_week' },
    {
      id: 'start_time',
      header: 'Start Time',
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/availabilities/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="availability" />
        </Flex>
