...
import { getInsuranceById } from 'apiSdk/insurances';
import { InsuranceInterface } from 'interfaces/insurance';
import { PatientListPage } from 'pages/patients';

function InsuranceViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<InsuranceInterface>(
    () => (id ? `/insurances/${id}` : null),
    () =>
      getInsuranceById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('insurance', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/insurances/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Coverage" text={data?.coverage} />

                <FormListItem label="Terms And Conditions" text={data?.terms_and_conditions} />
