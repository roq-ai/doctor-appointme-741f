...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    if (body?.availability?.length > 0) {
      const create_availability = body.availability;
      body.availability = {
        create: create_availability,
      };
    } else {
      delete body.availability;
    }
    if (body?.insurance?.length > 0) {
      const create_insurance = body.insurance;
      body.insurance = {
        create: create_insurance,
      };
    } else {
      delete body.insurance;
    }
