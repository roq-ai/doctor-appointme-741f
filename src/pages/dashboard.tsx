...
import { getAvailabilities } from 'apiSdk/availabilities';
...
import { InsuranceListPage as ShowCaseList } from 'pages/insurances';

function DashboardPage() {
  const { session } = useSession();

  const availabilityStat = useSWR<PaginatedInterface<AvailabilityInterface>>(
    () => `/availabilities`,
    () => getAvailabilities({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      ...
      result: availabilityStat?.data?.totalCount,
      isLoading: availabilityStat.isLoading,
    },
    {
      title: 'Appointments',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`insurances`} passHref legacyBehavior>
                  <Button
