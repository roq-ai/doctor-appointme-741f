...
export function PatientListPage(props: PatientListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getPatients({
        relations: ['user', 'insurance', 'appointment.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['gender.contains', 'medical_history.contains', 'emergency_contact.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'medical_history', header: 'Medical History', accessorKey: 'medical_history' },
    { id: 'emergency_contact', header: 'Emergency Contact', accessorKey: 'emergency_contact' },
    ...
    hasAccess('insurance', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/insurances/view/${record.insurance?.id}`}>
